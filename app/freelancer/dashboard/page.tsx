...
export default function FreelancerDashboardPage() {
  ...
  return (
    <ProtectedRoute allowedRole="freelancer">
      <div className="h-screen w-full overflow-hidden flex selection:bg-indigo-100 selection:text-indigo-900 bg-canvas">
        ...
        {/* Main Content Area (Fixed Top Header + Scrollable Main Content) */}
        <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
          <TopHeader onOpenMobileMenu={() => setIsMobileOpen(true)} />

          {/* Scrollable Main Freelancer Area */}
          <main className="flex-1 w-full overflow-y-auto pb-16">
          
          {/* Sub-header Banner dedicated for Freelancer (Only on Dashboard tab) */}
          {activeTab === 'dashboard' && (
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6">
                  <div>
                    ...
                    <h1 className="text-xl sm:text-3xl font-extrabold tracking-tight">
                      Welcome back, {(currentUser?.name && currentUser.name !== 'Gurpreet Singh') ? currentUser.name : (freelancer.name && freelancer.name !== 'Gurpreet Singh' ? freelancer.name : 'Badal Srari')}! 👋
                    </h1>
          ...
          {activeTab === 'escrow' && <EscrowSimulator inline />}
